...
It is **U**ncomplicated **F**ire**w**all. It's a security layer on Linux itself which manages:
- Which ports should be open
- Which ports should be closed
- What traffic to move through them

Checking if it's active:
```bash
ufw status
```
If it's inactive, no filter is applied on ports.
